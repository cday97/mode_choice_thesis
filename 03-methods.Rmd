...
```{r fig-asim-nest, fig.cap = "The nested logit model used in ActivitySim.", fig.asp=1, fig.align='center', out.extra = 'trim = {3cm 6.5cm 4.5cm 6cm}', out.width="1.8\\linewidth", wrapfigure = list("R", .5),  echo=FALSE}
#
...
\begin{align}
  V_{nk}^{Person} & = \alpha_{kn} + cost_{k,ij} (\beta_{kP}^1) + age_{n} 
  (\beta_{kP}^2) + hhsize_{n} (\beta_{kP}^3) (\#eq:person) \\
  V_{nk}^{Path} &= dist_{k,ij} (\beta_{kP}^4) + tv_{k,ij} (\beta_{kP}^5) + 
      te_{k} (\beta_{kP}^6) + tw_{k} (\beta_{kP}^7) + 
      prox_{k} (\beta_{kP}^8) + xfer_{k} (\beta_{kP}^9)  (\#eq:path) \\
  V_{nk}^{Location} &= ZDI_{k,i} (\beta_{kP}^{10}) + ZDI_{k,j} (\beta_{kP}^{11}) + 
      ZTI_{k,j} (\beta_{kP}^{12}) + CBD_{k,j} (\beta_{kP}^{13}) (\#eq:location)\\
...
\begin{equation}
  V_{nk} = \alpha_{k} + cost_{k,ij}(\beta_k^1) + tv_{k,ij}(\beta_k^2) +  xfer_{k} (\beta_k^3) (\#eq:beam)
\end{equation}
