...
\begin{equation}
 V_{Path} = \beta_{TV}(T_{Vehicle}) + \beta_{TW}(T_{Wait}) + \beta_{TE}(T_{Egress}) + \beta_{TP}(TransitProx) + \\ \beta_{xfer}(xfer) + \beta_{DW}(D_{Walk}) + \beta_{DB}(D_{Bike}) + \beta_{DD}(D_{Drive}) (\#eq:path)
\end{equation}
...
\begin{equation}  
  V_{Mode} = V_{Person} + V_{Path} + V_{Location} (\#eq:all)
\end{equation}
